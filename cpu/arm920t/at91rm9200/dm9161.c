...
unsigned int dm9161_IsPhyConnected (AT91PS_EMAC p_mac)
{
	...
	at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, DM9161_PHYID1, &Id1);
	at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, DM9161_PHYID2, &Id2);
...
UCHAR dm9161_GetLinkSpeed (AT91PS_EMAC p_mac)
{
	...
	if (!at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, DM9161_BMSR,
		&stat1))
		return FALSE;
	...
	if (!at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, DM9161_DSCSR,
		&stat2))
		return FALSE;
...
UCHAR dm9161_InitPhy (AT91PS_EMAC p_mac)
{
	...
	at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, DM9161_MDINTR,
		&IntValue);
	/* set FDX, SPD, Link, INTR masks */
	IntValue |= (DM9161_FDX_MASK | DM9161_SPD_MASK |
		     DM9161_LINK_MASK | DM9161_INTR_MASK);
	at91rm9200_EmacWritePhy (p_mac, CONFIG_PHY_ADDR, DM9161_MDINTR,
		&IntValue);
	at91rm9200_EmacDisableMDIO (p_mac);
...
UCHAR dm9161_AutoNegotiate (AT91PS_EMAC p_mac, int *status)
{
	...
	if (!at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, DM9161_BMCR,
		&value))
		return FALSE;
	value &= ~DM9161_AUTONEG;	/* remove autonegotiation enable */
	value |= DM9161_ISOLATE;	/* Electrically isolate PHY */
	if (!at91rm9200_EmacWritePhy (p_mac, CONFIG_PHY_ADDR, DM9161_BMCR,
		&value))
		return FALSE;
	...
	if (!at91rm9200_EmacWritePhy (p_mac, CONFIG_PHY_ADDR, DM9161_ANAR,
		&PhyAnar))
		return FALSE;
	...
	udelay (10000);
	at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, DM9161_BMSR, &value);
	if (!(value & DM9161_AUTONEG_COMP))
		return FALSE;

	/* Get the AutoNeg Link partner base page */
	if (!at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, DM9161_ANLPAR,
		&PhyAnalpar))
		return FALSE;
