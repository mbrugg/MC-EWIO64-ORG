...
unsigned int lxt972_IsPhyConnected (AT91PS_EMAC p_mac)
{
	...
	at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, PHY_COMMON_ID1, &Id1);
	at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, PHY_COMMON_ID2, &Id2);
...
UCHAR lxt972_GetLinkSpeed (AT91PS_EMAC p_mac)
{
	...
	if (!at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, PHY_LXT971_STAT2,
		&stat1))
		return FALSE;
...
UCHAR lxt972_InitPhy (AT91PS_EMAC p_mac)
{
	...
	at91rm9200_EmacWritePhy (p_mac, CONFIG_PHY_ADDR, PHY_LXT971_INT_ENABLE,
		0);
...
UCHAR lxt972_AutoNegotiate (AT91PS_EMAC p_mac, int *status)
{
	...
	if (!at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, PHY_COMMON_CTRL,
		&value))
		return FALSE;

	/* Restart Auto_negotiation  */
	value |= PHY_COMMON_CTRL_RES_AUTO;
	if (!at91rm9200_EmacWritePhy (p_mac, CONFIG_PHY_ADDR, PHY_COMMON_CTRL,
		&value))
		return FALSE;

	/*check AutoNegotiate complete */
	udelay (10000);
	at91rm9200_EmacReadPhy (p_mac, CONFIG_PHY_ADDR, PHY_COMMON_STAT,
		&value);
