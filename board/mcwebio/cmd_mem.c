 ...
 */
/*
 * 08.02.2010 Modivied MCWEBIO Flash 64 MByte Karl-Heinz Weber KWeber@metz-connect.com
 */
...
#ifdef CFG_FLASH_CFI_DRIVER_MC

//extern unsigned long crc32mc (unsigned long, const unsigned char *, unsigned int);
...
int do_mem_md ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	do {
		...
		for (i=0; i<linebytes; i+= size) {
			...
			#ifdef CFG_FLASH_CFI_DRIVER_MC
			// check if we  adress to Flash and set pio A24
			addr2info(addr);
			#endif
...
//	if((addr2info(addr2))&&(addr2info(addr1)))
//		{
//		printf("both flash address, no copy");
//		return 1;
//		}
#endif

	count = simple_strtoul(argv[3], NULL, 16);
if((addr1&0x0000000f)!=0x0)
	if((addr1&0x0000000f)!=0x4)
		if((addr1&0x0000000f)!=0x8)
			if((addr1&0x0000000f)!=0xc)
				{
				printf("address %x not word based\n",addr1);
				return 1;
				}
if((addr2&0x0000000f)!=0x0)
	if((addr2&0x0000000f)!=0x4)
		if((addr2&0x0000000f)!=0x8)
			if((addr2&0x0000000f)!=0xc)
				{
				printf("address %x not word based\n",addr2);
				return 1;
				}
