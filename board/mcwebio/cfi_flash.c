...
uchar * ret=(uchar *)(info->start[sect] + (offset * info->portwidth));
set_addr24(ret);
return ret;
...
//	udelay(3000);
	// Set Output Data Register of the pin (value ? PIO_SODR : PIO_CODR)
//	AT91C_BASE_PIOC->PIO_SODR = AT91C_PIO_PC8;
//udelay(1500000);
...
//printf("flash_read_user_serial from %x to %x (src %x offset %x) Anzahl %d",dst,src + offset,src,offset, len);
